...
const statConfig = [
  { key: "cards", icon: Layers3, href: "/admin/cards" },
  { key: "seasons", icon: Sparkles, href: "/admin/seasons" },
  { key: "sets", icon: Boxes, href: "/admin/sets" },
  { key: "rarities", icon: Diamond, href: "/admin/rarities" },
  { key: "types", icon: Shapes, href: "/admin/types" },
  { key: "glossary", icon: BookOpenText, href: "/admin/glossary" },
] as const;

export default async function AdminDashboardPage() {
  const [t, common, locale, cards, seasons, sets, rarities, types, glossary] = await Promise.all([
  ...
  const quickActions = [
    [t("addCard"), "/admin/cards/new"],
    [t("addSeason"), "/admin/seasons/new"],
    [t("addSet"), "/admin/sets/new"],
    [t("addGlossary"), "/admin/glossary/new"],
  ] as const;
  ...
  return (
    <>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {statConfig.map(({ key, icon: Icon, href }) => (
          <Link href={href} key={key} className="group flex items-center justify-between rounded-xl border bg-card p-5 transition hover:border-safir/30 hover:shadow-lg hover:shadow-safir/5">
            <div><p className="text-xs font-medium text-muted-foreground">{t(`stats.${key}`)}</p><p className="mt-1 font-heading text-3xl font-semibold tabular-nums">{counts[key]}</p></div>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
        <section className="overflow-hidden rounded-xl border bg-card">
          <div className="flex items-center justify-between border-b px-5 py-4"><div><h2 className="font-heading text-xl font-semibold">{t("recentCards")}</h2><p className="text-xs text-muted-foreground">{t("latestActivity")}</p></div><Link href="/admin/cards" className="flex items-center gap-1 text-xs font-medium text-safir hover:underline">{common("viewAll")} <ArrowRight className="size-3.5" /></Link></div>
          ...
          ))}</div> : <p className="px-5 py-12 text-center text-sm text-muted-foreground">{t("noCards")}</p>}
        </section>
        <section className="rounded-xl border bg-card p-5">
          <h2 className="font-heading text-xl font-semibold">{t("quickActions")}</h2>
          <p className="mt-0.5 mb-4 text-xs text-muted-foreground">{t("quickActionsDescription")}</p>
