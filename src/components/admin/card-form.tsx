  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <label className="admin-label" htmlFor={`description-${locale}`}>Description</label>
        <Button type="button" variant="ghost" size="xs" onClick={() => setPreview((current) => !current)}><Eye /> {preview ? "Edit" : "Preview"}</Button>
      </div>
      {preview ? (
          ...
          {value ? tokenizeGlossaryContent(value).map((token, index) => {
            ...
            return (
              <span
                ...
                title={translation?.definition ?? `Unknown glossary term: ${token.key}`}
              >
        ...
        <textarea
          ...
          placeholder="Describe the card. Insert glossary terms with [[key]]."
        />
      )}
      <div className="flex flex-col gap-2 rounded-lg border bg-muted/20 p-2 sm:flex-row">
        <select className="admin-input h-8 flex-1" value={selected} onChange={(event) => setSelected(event.target.value)} aria-label="Glossary term">
          <option value="">Insert glossary term…</option>
          {glossary.map((entry) => (
            <option key={entry.id} value={entry.key}>{getTranslation(entry.translations, locale)?.label ?? entry.key}</option>
          ))}
        </select>
        <Button type="button" variant="outline" size="sm" onClick={insertReference} disabled={!selected}><Plus /> Insert</Button>
      </div>
    </div>
  );
}
...
export function CardForm({
  ...
  useEffect(() => {
    ...
    const notice = encodeURIComponent(state.message ?? "Card saved.");
    ...
    <form action={formAction} className="space-y-5" onChangeCapture={() => setDirty(true)}>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="space-y-5">
          <FormSection title="General" description="Core identifiers and catalogue placement.">
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="space-y-2">
                <label className="admin-label" htmlFor="card-number">Number</label>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <label className="admin-label" htmlFor="card-slug">Slug</label>
                  {mode === "create" ? (
                    <label className="flex items-center gap-1.5 text-xs text-muted-foreground"><input type="checkbox" checked={slugAutomatic} onChange={(event) => setSlugAutomatic(event.target.checked)} /> Auto</label>
                  ) : null}
                </div>
              ...
              <div className="space-y-2">
                <label className="admin-label" htmlFor="card-season">Season</label>
                <select
                  id="card-season"
                ...
                {fieldError("seasonId") ? <p className="admin-error">Please select a season.</p> : null}
              </div>
              <div className="space-y-2">
                <label className="admin-label" htmlFor="card-set">Set <span className="font-normal text-muted-foreground">(optional)</span></label>
                ...
                {setResetNotice ? <p className="text-xs text-amber-600 dark:text-amber-400">The previous set was cleared because it belongs to another season.</p> : null}
              </div>
            </div>
          </FormSection>

          <FormSection title="Classification" description="Rarity and one or more gameplay types.">
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2">
                <label className="admin-label" htmlFor="card-rarity">Rarity</label>
              ...
              <fieldset className="space-y-3">
                <legend className="admin-label">Types</legend>
                ...
                {value.typeIds.length ? <div className="flex flex-wrap gap-1.5">{value.typeIds.map((id) => <Badge variant="secondary" key={id}>{types.find((type) => type.id === id)?.label ?? id}</Badge>)}</div> : <p className="admin-help">No type selected.</p>}
              </fieldset>
            </div>
          </FormSection>

          <FormSection title="Stats" description="Each gameplay statistic must be an integer from 0 to 9.">
              ...
              {(["attack", "value", "defense"] as const).map((stat) => {
                ...
                return (
                  <div className="rounded-xl border bg-muted/20 p-4" key={stat}>
                    <label className="mb-3 block text-xs font-semibold tracking-[0.12em] uppercase" htmlFor={`stat-${stat}`}>{stat}</label>
                    ...
                    {invalid ? <p className="admin-error mt-2">Must be between 0 and 9.</p> : null}
                  </div>
                );
              })}
            </div>
          ...
          <FormSection title="Card flags" description="Special catalogue and gameplay behaviours.">
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                ["isCommander", "Commander", "This card can be used as a Commander."],
                ["isPromo", "Promo", "This card belongs to a promotional release."],
                ["isFeatured", "Featured", "Highlight this card in curated surfaces."],
          ...
          <FormSection title="Translations" description="Choose a primary language; incomplete languages are clearly marked and may be completed later.">
            <div className="mb-5 max-w-xs space-y-2">
              <label className="admin-label" htmlFor="primary-locale">Primary language</label>
            ...
            {SUPPORTED_LOCALES.map((locale) => (
              <div className="space-y-5" key={locale} hidden={activeLocale !== locale} role="tabpanel">
                <div className="space-y-2">
                  <label className="admin-label" htmlFor={`name-${locale}`}>Name</label>
          ...
          <FormSection title="Alternative artworks" description="Each artwork has an independent stable ID and explicit display order.">
            <div className="space-y-4">
              {value.alternativeArtworks.map((artwork, index) => (
                <div className="rounded-xl border bg-muted/15 p-4" key={artwork.id}>
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div><p className="text-sm font-semibold">Alternative #{index + 1}</p><p className="font-mono text-[10px] text-muted-foreground">{artwork.id}</p></div>
                    <div className="flex gap-1">
                      <Button type="button" variant="ghost" size="icon-sm" disabled={index === 0} onClick={() => moveAlternative(index, -1)} aria-label="Move up"><ArrowUp /></Button>
                      <Button type="button" variant="ghost" size="icon-sm" disabled={index === value.alternativeArtworks.length - 1} onClick={() => moveAlternative(index, 1)} aria-label="Move down"><ArrowDown /></Button>
                      <Button type="button" variant="destructive" size="icon-sm" onClick={() => removeAlternative(artwork.id)} aria-label="Remove artwork"><Trash2 /></Button>
                    </div>
                  </div>
                  <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_15rem]">
                    ...
                    <div className="space-y-3"><p className="admin-label">Orientation</p><OrientationPicker value={artwork.orientation} onChange={(orientation) => updateAlternative(artwork.id, { orientation })} /><p className="admin-help">Order: {index}</p></div>
                  </div>
                </div>
              ))}
              {value.alternativeArtworks.length === 0 ? <p className="rounded-xl border border-dashed p-6 text-center text-sm text-muted-foreground">No alternative artwork.</p> : null}
              <Button type="button" variant="outline" onClick={addAlternative}><ImagePlus /> Add Alternative Artwork</Button>
            </div>
          </FormSection>

          {mode === "edit" && value.id ? (
            <FormSection title="System information" description="Read-only identifiers useful for support and debugging.">
              <dl className="grid gap-4 text-sm sm:grid-cols-3">
                <div><dt className="text-xs text-muted-foreground">Document ID</dt><dd className="mt-1 break-all font-mono text-xs">{value.id}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Created</dt><dd className="mt-1">{value.createdAt ?? "—"}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Updated</dt><dd className="mt-1">{value.updatedAt ?? "—"}</dd></div>
              </dl>
            </FormSection>
          ) : null}
        </div>
        ...
        <aside className="space-y-5 xl:sticky xl:top-24 xl:self-start">
          <FormSection title="Main artwork" description="The original image is never cropped in Admin.">
            <div className="space-y-4">
              ...
              {!value.artwork.storagePath && fieldError("artwork.storagePath") ? <p className="admin-error">A main artwork is required.</p> : null}
            </div>
          </FormSection>
          <div className="rounded-xl border bg-safir/5 p-4 text-sm">
            <p className="flex items-center gap-2 font-semibold text-safir"><Sparkles className="size-4" /> Card summary</p>
            <p className="mt-2 font-heading text-xl font-semibold">{value.translations[primaryLocale]?.name || "Untitled card"}</p>
      ...
      <FormActions cancelHref="/admin/cards" pending={pending} label={mode === "create" ? "Save Card" : "Save changes"} />
    </form>
  );
}
