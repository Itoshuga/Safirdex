...
export function SetForm({ action, mode, seasons, initial }: { action: FormAction; mode: "create" | "edit"; seasons: Option[]; initial?: SetFormValue }) {
  ...
  useEffect(() => { if (state.status === "success") { router.push(`/admin/sets?notice=${encodeURIComponent(state.message ?? "Set saved.")}`); router.refresh(); } }, [router, state]);
  ...
  return (
    <form action={formAction} className="space-y-5" onChangeCapture={() => setDirty(true)}>
      ...
      <FormSection title="General" description="A set always belongs to exactly one season.">
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-2"><label className="admin-label" htmlFor="set-season">Season</label><select id="set-season" className="admin-input" value={value.seasonId} onChange={(event) => update({ seasonId: event.target.value })}><option value="">Select a season</option>{seasons.map((season) => <option key={season.id} value={season.id}>{season.label}</option>)}</select></div>
          <div className="space-y-2"><label className="admin-label" htmlFor="set-code">Code</label><input id="set-code" className="admin-input uppercase" maxLength={32} value={value.code} onChange={(event) => update({ code: event.target.value.toUpperCase() })} /></div>
          <div className="space-y-2"><div className="flex justify-between"><label className="admin-label" htmlFor="set-slug">Slug</label>{mode === "create" ? <label className="text-xs text-muted-foreground"><input className="mr-1" type="checkbox" checked={autoSlug} onChange={(event) => setAutoSlug(event.target.checked)} />Auto</label> : null}</div><input id="set-slug" className="admin-input" value={value.slug} onChange={(event) => { setAutoSlug(false); update({ slug: event.target.value }); }} /></div>
          <div className="space-y-2"><label className="admin-label" htmlFor="set-release">Release date</label><input id="set-release" className="admin-input" type="date" value={value.releaseDate} onChange={(event) => update({ releaseDate: event.target.value })} /></div>
        </div>
      </FormSection>
      ...
      <FormActions cancelHref="/admin/sets" pending={pending} label={mode === "create" ? "Save Set" : "Save changes"} />
    </form>
  );
}
